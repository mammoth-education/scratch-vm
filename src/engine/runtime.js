...
const MjpgStreamer = require('../io/mjpgStreamer');
...
class Runtime extends EventEmitter {
     ...
     * @return {string} - the name of the device.
     */
    getDeviceInfo(extensionId) {
        let info = null;
        if (this.peripheralExtensions[extensionId]) {
            info = this.peripheralExtensions[extensionId].getDeviceInfo();
        }
        return info;
    }
    /**
     ...
     * @return {string} - the name of the device.
     */
    getReceiveBuffer(extensionId) {
        let receiveBuffer = null;
        if (this.peripheralExtensions[extensionId]) {
            receiveBuffer = this.peripheralExtensions[extensionId].getReceiveBuffer();
        }
    ...
    /**
     * Setting up a device to connect to WiFi.
     * @param {obj} data - WiFi name and password.
     */
    settingDeviceWiFi(extensionId, data) {
        if (this.peripheralExtensions[extensionId]) {
    ...
    calibration(extensionId, state) {
        if (this.peripheralExtensions[extensionId]) {
            this.peripheralExtensions[extensionId].calibration(state);
        }
    }

    /**
     * GetDevicesWifiData.
     * @param {obj} data - WiFi IP.
     */
    getDevicesWifiData(extensionId) {
        if (this.peripheralExtensions[extensionId]) {
            this.peripheralExtensions[extensionId].getDevicesWifiInfo();
        }
    }
